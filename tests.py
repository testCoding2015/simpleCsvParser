...
class MyCsvTest(unittest.TestCase):
	...
	res_cells = [ ['John D', '120 any st.', '"Anytown, WW"', '08123', 'Andrew P', '114 Sansome st.'
			    ...
			    , ['"For whom the bells toll"', '0', '0', '"Bring me some shrubbery"', '2', '3', '"Once upon \r\na time"'
			      , '5', '6', '"\'It\'s just a flesh wound."', '8', '9']]
	...
	def test_get_cell(self):
		self.failUnless([ [self.csv.get_cell(i,j) for i, j in itertools.product(range(3), range(4))]
						  ,[self.csv2.get_cell(i,j) for i, j in itertools.product(range(4), range(3))] ] == self.res_cells)
	...
	def test_infer_types(self):
		self.failUnless([self.csv.infer_types(), self.csv2.infer_types(), self.csv_inference.infer_types() ] == self.res_types)
